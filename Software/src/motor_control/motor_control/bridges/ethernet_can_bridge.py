...
IMU handling is one-way: STM32 -> ROS only.
This node never sends IMU data or IMU requests from ROS back to the STM32.
...
from motor_control.common import get_software_log_dir
...
class EthernetCanBridge(Node):
    def __init__(self):
        ...
        self.declare_parameter('publish_stm32_imu',  True)
        ...
        self.publish_stm32_imu = bool(self.get_parameter('publish_stm32_imu').value)
        ...
        if self.publish_stm32_imu:
            self.imu_publisher = self.create_publisher(Imu, self.imu_topic, 10)

        # Control channel only (/motor_can_tx -> STM32). No ROS->STM32 IMU channel.
        ...
        if self.all_logging:
            self.get_logger().info(
                ...
                f'publish_stm32_imu={self.publish_stm32_imu}'
            )
    ...
    def _handle_line(self, line: str):
        ...
        if parts[0] == 'FBK' and len(parts) >= 4:
            ...
            # Forward sensor data from STM32 to ROS only.
            self._handle_imu_line(parts, line)

        else:
            if self.all_logging:
                self.get_logger().warn(f'Unsupported UDP line: {line}')

    def _handle_imu_line(self, parts: list[str], raw_line: str) -> None:
        if self.imu_publisher is None:
            return

        # Supported formats from STM32 text stream:
        #   IMU qw qx qy qz gx gy gz ax ay az
        #   IMU gx gy gz ax ay az
        try:
            values = [float(x) for x in parts[1:]]
        except ValueError:
            self.get_logger().warning(f'Invalid IMU line (non-float): {raw_line}')
            return

        if len(values) == 10:
            qw, qx, qy, qz, gx, gy, gz, ax, ay, az = values
        elif len(values) == 6:
            qw, qx, qy, qz = 1.0, 0.0, 0.0, 0.0
            gx, gy, gz, ax, ay, az = values
        else:
            self.get_logger().warning(
                f'Invalid IMU line length ({len(values)} values): {raw_line}'
            )
            return

        # Normalise quaternion to avoid propagating malformed orientation.
        ...
        msg.header.frame_id = str(self.imu_frame_id)
        ...
        if self.all_logging:
            self.get_logger().info(
                f'Published IMU: gyro=({gx:.3f},{gy:.3f},{gz:.3f}) '
