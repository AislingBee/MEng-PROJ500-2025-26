    ...
    def run(self) -> None:
        ...
        for idx, mid in enumerate(self._motor_ids):
            ...
            self.get_logger().info(
                ...
                f" — ramping to 0.0 rad over {self._ramp_time_s:.1f}s"
            )

            # Ramp this motor: kp/kd scale from 0 → full over ramp_time_s so
            # torque builds gradually. q_des is 0.0 throughout.
