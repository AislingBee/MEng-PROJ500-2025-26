...
# Run:   ros2 launch motor_control single_motor_launch.py
#
# Pipeline:
#   single_motor_control → /robot_command → robot_command_bridge
#       → /motor_can_tx → serial_can_bridge (AT-frame binary)
#       → STM32 USART3 → CAN bus → RS04 motor feedback
#       → motor_feedback_listener → /motor_feedback → single_motor_control
#
...
def generate_launch_description():
    # 1. single_motor_control: publishes a ±target_q step command and logs feedback
    single_motor_control_node = Node(
        package='motor_control',
        executable='single_motor_control.py',
        name='single_motor_control',
        output='screen',
        parameters=[{
