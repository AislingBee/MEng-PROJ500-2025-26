...
# Run:   ros2 launch motor_control multi_motor_launch.py
#
# Pipeline (same as single_motor but N motors):
#   multi_motor_control → /robot_command → robot_command_bridge
...
#       → /motor_feedback → multi_motor_control (logs position)
#
...
_RATE_HZ       = 60.0   # command publish rate [Hz]
...
def generate_launch_description():
    # 1. multi_motor_control: publishes ±target_q step commands for all N joints
    multi_motor_control_node = Node(
        package='motor_control',
        executable='multi_motor_control.py',
        name='multi_motor_control',
        output='screen',
        parameters=[{
