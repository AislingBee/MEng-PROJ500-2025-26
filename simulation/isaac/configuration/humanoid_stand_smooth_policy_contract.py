...
if sum(OBS_LAYOUT.values()) != OBS_DIM:
    raise RuntimeError(
        f"Smooth standing observation layout sums to {sum(OBS_LAYOUT.values())}, expected {OBS_DIM}"
    )
if ACTION_DIM != 12:
...
class HumanoidStandSmoothPolicyContract:
...
CONTRACT: Final[HumanoidStandSmoothPolicyContract] = HumanoidStandSmoothPolicyContract()
