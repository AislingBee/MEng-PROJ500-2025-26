...
# Walking sim-to-real policy contract.
# Keep deployment-critical constants aligned between walking training and Thor.
...
DEFAULT_GAIT_FREQUENCY_HZ: Final[float] = 1.0
...
if sum(OBS_LAYOUT.values()) != WALKING_OBS_DIM:
    raise RuntimeError(
        f"Walking observation layout sums to {sum(OBS_LAYOUT.values())}, expected {WALKING_OBS_DIM}"
    )
...
class WalkingS2RPolicyContract:
...
CONTRACT: Final[WalkingS2RPolicyContract] = WalkingS2RPolicyContract()
