...
parser = argparse.ArgumentParser(description="Play back smooth deployable humanoid walking S2R PPO checkpoint.")
...
parser.add_argument(
    ...
    type=str,
    default=None,
    help="Path to a checkpoint file or run directory. Defaults to the latest smooth walking checkpoint.",
)
...
LOG_ROOT = REPO_ROOT / "logs" / "rsl_rl" / "humanoid_walk_smooth"
...
def resolve_checkpoint_path(raw_checkpoint: str | None) -> Path:
    ...
    if not LOG_ROOT.exists():
        raise FileNotFoundError(
            f"No smooth walking log directory found at {LOG_ROOT.resolve()}. "
    ...
    raise FileNotFoundError(
        f"No smooth walking checkpoints found under {LOG_ROOT.resolve()}. "
        "Pass --checkpoint explicitly to choose a checkpoint."
    )
...
def main():
    ...
    obs, _ = env.reset()
    step_count = 0
    print("Starting smooth walking playback...")
