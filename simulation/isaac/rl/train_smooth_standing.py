...
# This script trains only the smooth, non-saturating standing S2R policy.
# It is intentionally separate from the existing walking policy launchers and
# should not be used for walking training or walking policy export.
parser = argparse.ArgumentParser(description="Train PROJ500 smooth standing S2R PPO.")
...
def export_deployable_policy(runner, export_dir: str) -> None:
    ...
    if normalizer is None:
        print("WARNING: observation normalization is enabled, but no runner normalizer was found to wrap.")


def _strip_deprecated_model_fields(cfg_dict: dict) -> None:
...
def main():
    ...
    if args.task != "Humanoid-Stand-Smooth-S2R-v0":
        raise RuntimeError("train_smooth_standing.py is dedicated to Humanoid-Stand-Smooth-S2R-v0 only.")
    ...
    agent_cfg.experiment_name = "humanoid_stand_smooth_s2r"
    ...
    _strip_deprecated_model_fields(cfg_dict)
